                ...
                "content": [
                    {
                        ...
                        "text": (
                            "# Role\n"
                            "你是一位专业的无人机安全分析师与渗透测试专家"
                            "（UAV Security Analyst & Penetration Tester）。\n"
                            "你的任务是通过分析以下无人机运行、展示或实验的视频内容，评估并总结"
                            "该无人机系统（UAS）在物理、通信、导航及操作层面潜在的安全脆弱性"
                            "（Vulnerabilities）。\n"
                            "\n"
                            "# Reference Framework\n"
                            "评估时，请参考以下无人机安全脆弱性框架，从视频画面的视觉线索中捕捉相关信息：\n"
                            "1. 物理与硬件安全：设备暴露的物理接口（如USB、调试串口）、"
                            "天线类型与布局、物理防篡改能力。\n"
                            "2. 传感器与导航安全：GPS天线的可见性、是否在强电磁/干扰风险区域飞行、"
                            "有无冗余导航传感器。\n"
                            "3. 通信与地面站控制（若画面中出现地面站屏幕、FPV画面或遥控器）："
                            "遥控信号指示、是否显示明文遥测数据、链路连接状态指标、"
                            "有无未加密/未授权连接风险。\n"
                            "4. 运行环境安全：操作员与无人机的物理距离、视距（VLOS）情况、"
                            "操作环境对视线或信号的阻挡。\n"
                            "\n"
                            "# Instructions\n"
                            "请仔细观看并分析视频，完成以下任务：\n"
                            "1. **视频内容描述**：描述你观察到的视频内容。\n"
                            "2. **视觉线索提取**：描述你在视频中观察到的无人机型号、硬件配置、"
                            "运行环境以及地面控制站（如有）的关键视觉细节。\n"
                            "3. **脆弱性识别与分析**：结合观察到的线索，总结出视频中所展示的"
                            "安全脆弱性。请重点关注：\n"
                            "   - 是否容易受到 GPS 干扰/欺骗（GPS Jamming/Spoofing）？\n"
                            "   - 无线通信链路（如 MAVlink）是否存在被监听（Eavesdropping）"
                            "或信号注入（Injection）的风险？\n"
                            ...
                            "- 如果视频中没有足够明显的信息来判断某个脆弱性，请明确说明"
                            "「视频画面中未观察到，无法判断」。\n"
                            "- 不要假设视频之外的内容（如无人机的品牌、型号、通信协议），"
                            "只描述实际看到的东西。"
                        ),
                    },
